...
import numpy as np
import cross_entropy_loss as cel
...
class ResampleLoss(nn.Module):
    ...
    def __init__(self,
        ...
        if self.use_sigmoid:
            if self.partial:
                self.cls_criterion = cel.partial_cross_entropy()
            else:
                self.cls_criterion = binary_cross_entropy
        else:
            self.cls_criterion = cel.cross_entropy()
    ...
    def forward(self,
        ...
        if self.focal:
            ...
            alpha_t = torch.where(label==1, torch.tensor(self.alpha).cuda(), torch.tensor(1-self.alpha).cuda())
